  ...
  proof -
    ...
    proof (rule assoc_mult_mat)
      ...
      show "|Deutsch.zero\<rangle> + exp (2 * \<i> * complex_of_real pi *  complex_of_nat (j div 2) /
        ...
        by (simp add: carrier_matI index_unit_vec(3) state_basis_def)
    qed
    also have "\<dots> = (((1\<^sub>m 2) \<Otimes> SWAP_down (Suc n)) * (control2 (R (Suc (Suc n))) \<Otimes> (1\<^sub>m (2^n)))) * 
          (((1\<^sub>m 2) \<Otimes> SWAP_up (Suc n)) * ( |Deutsch.zero\<rangle> + 
          exp (2 * \<i> * complex_of_real pi * complex_of_nat (j div 2) / 2 ^ Suc n) \<cdot>\<^sub>m |Deutsch.one\<rangle> \<Otimes>
...
next
  ...
  next
    ...
    proof -
      ...
      hence a5:"dim_col (1\<^sub>m 2) > 0" by simp
...
next 
  ...
  proof -
    ...
    have a5:"dim_col (1\<^sub>m 2) > 0" using c3 by auto
    have a6:"dim_row (f (Suc k)) > 0" using r4 by auto
    show ?thesis using a1 a2 a3 a4 a5 a6 mult_distr_tensor
...
proof
  ...
  thus "\<And>i j. i < dim_row m \<Longrightarrow> j < dim_col m \<Longrightarrow> R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)"
  proof -
    fix i j
    assume "i < dim_row m"
    ...
    next
      ...
      next 
        ...
        show "R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)" 
        proof -
          have "R k\<^sup>\<dagger> $$ (0,0) = cnj (R k $$ (0,0))" 
            using dagger_def
      ...
      next
        ...
        show "R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)"
        proof -
          have "R k\<^sup>\<dagger> $$ (0,1) = cnj (R k $$ (1,0))"
            using dagger_def 
            by (metis (no_types, lifting) One_nat_def R_def Suc_1 Suc_eq_plus1 
    ...
    next
      ...
      next 
        ...
        show "R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)"
        proof -
          have "R k\<^sup>\<dagger> $$ (1,0) = cnj (R k $$ (0,1))"
            using dagger_def
            by (metis (no_types, lifting) One_nat_def R_def Suc_1 Suc_eq_plus1 
      ...
      next
        ...
        show "R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)" 
        proof -
          have "R k\<^sup>\<dagger> $$ (1,1) = cnj (R k $$ (1,1))"
            using dagger_def
            by (metis (no_types, lifting) One_nat_def R_def Suc_1 Suc_eq_plus1 
                Tensor.mat_of_cols_list_def dim_col_mat(1) dim_row_mat(1) index_mat(1) 
                less_Suc_numeral list.size(3) list.size(4) old.prod.case power_eq_0_iff 
          ...
          finally have "R k\<^sup>\<dagger> $$ (i, j) = m $$ (i, j)" using i1 j1 by simp
          thus ?thesis by this
        qed
      qed
    qed
...
proof
  ...
  show "unitary (R n)"
  proof -
    have "((R n)\<^sup>\<dagger>) * (R n) = 1\<^sub>m 2 \<and> (R n) * ((R n)\<^sup>\<dagger>) = 1\<^sub>m 2"
    proof
      show "R n\<^sup>\<dagger> * R n = 1\<^sub>m 2"
      proof
        show "\<And>i j. i < dim_row (1\<^sub>m 2) \<Longrightarrow> j < dim_col (1\<^sub>m 2) \<Longrightarrow> 
              (R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
        proof -
          fix i j
          ...
          hence j2:"j < 2" by auto
          show "(R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
          proof (rule disjE)
            show "i = 0 \<or> i = 1" using i2 by auto
          next
            assume i0:"i = 0"
            ...
            next
              ...
              show "(R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n\<^sup>\<dagger> * R n) $$ (0,0) = ((R n)\<^sup>\<dagger> $$ (0,0)) * ((R n) $$ (0,0)) +
                      ((R n)\<^sup>\<dagger> $$ (0,1)) * ((R n) $$ (1,0))"
            ...
            next
              ...
              show "(R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)" 
              proof -
                have "(R n\<^sup>\<dagger> * R n) $$ (0,1) = ((R n)\<^sup>\<dagger> $$ (0,0)) * ((R n) $$ (0,1)) +
                      ((R n)\<^sup>\<dagger> $$ (0,1)) * ((R n) $$ (1,1))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
          ...
          next
            ...
            next
              ...
              show "(R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n\<^sup>\<dagger> * R n) $$ (1,0) = ((R n)\<^sup>\<dagger> $$ (1,0)) * ((R n) $$ (0,0)) +
                      ((R n)\<^sup>\<dagger> $$ (1,1)) * ((R n) $$ (1,0))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
                also have "\<dots> = 0" using R_dagger_mat R_def index_mat_of_cols_list
            ...
            next
              ...
              show "(R n\<^sup>\<dagger> * R n) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n\<^sup>\<dagger> * R n) $$ (1,1) = ((R n)\<^sup>\<dagger> $$ (1,0)) * ((R n) $$ (0,1)) +
                      ((R n)\<^sup>\<dagger> $$ (1,1)) * ((R n) $$ (1,1))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
        ...
        qed
      next
        show "dim_row (R n\<^sup>\<dagger> * R n) = dim_row (1\<^sub>m 2)"
          using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> by auto
      next
        show "dim_col (R n\<^sup>\<dagger> * R n) = dim_col (1\<^sub>m 2)"
          using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> by auto
      qed
    next
      show "R n * ((R n)\<^sup>\<dagger>) = 1\<^sub>m 2"
      proof
        show "\<And>i j. i < dim_row (1\<^sub>m 2) \<Longrightarrow> j < dim_col (1\<^sub>m 2) \<Longrightarrow>
              (R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
        proof -
          fix i j
          assume "i < dim_row (1\<^sub>m 2)"
          hence i2:"i < 2" by auto
          assume "j < dim_col (1\<^sub>m 2)"
          hence j2:"j < 2" by auto
          show "(R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
          proof (rule disjE)
            show "i = 0 \<or> i = 1" using i2 by auto
          next
            assume i0:"i = 0"
            ...
            next
              ...
              show "(R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n * (R n\<^sup>\<dagger>)) $$ (0,0) = ((R n) $$ (0,0)) * ((R n)\<^sup>\<dagger> $$ (0,0)) +
                      ((R n) $$ (0,1)) * ((R n)\<^sup>\<dagger> $$ (1,0))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
            ...
            next
              ...
              show "(R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n * (R n\<^sup>\<dagger>)) $$ (0,1) = ((R n) $$ (0,0)) * ((R n)\<^sup>\<dagger> $$ (0,1)) +
                      ((R n) $$ (0,1)) * ((R n)\<^sup>\<dagger> $$ (1,1))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
          ...
          next
            ...
            next
              ...
              show "(R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)"
              proof -
                have "(R n * (R n\<^sup>\<dagger>)) $$ (1,0) = ((R n) $$ (1,0)) * ((R n)\<^sup>\<dagger> $$ (0,0)) +
                      ((R n) $$ (1,1)) * ((R n)\<^sup>\<dagger> $$ (1,0))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
            ...
            next
              ...
              show "(R n * (R n\<^sup>\<dagger>)) $$ (i, j) = 1\<^sub>m 2 $$ (i, j)" 
              proof -
                have "(R n * (R n\<^sup>\<dagger>)) $$ (1,1) = ((R n) $$ (1,0)) * ((R n)\<^sup>\<dagger> $$ (0,1)) +
                      ((R n) $$ (1,1)) * ((R n)\<^sup>\<dagger> $$ (1,1))"
                  using \<open>dim_row (R n) = 2 ^ 1\<close> \<open>square_mat (R n)\<close> sumof2 by fastforce
                also have "\<dots> = exp(2*pi*\<i>/2^n) * exp(-2*pi*\<i>/2^n)"
                  using R_dagger_mat R_def index_mat_of_cols_list by simp
                also have "\<dots> = 1"
        ...
        qed
      next
        show "dim_row (R n * (R n\<^sup>\<dagger>)) = dim_row (1\<^sub>m 2)"
          by (simp add: \<open>dim_row (R n) = 2 ^ 1\<close>)
      next
        show "dim_col (R n * (R n\<^sup>\<dagger>)) = dim_col (1\<^sub>m 2)"
          by (simp add: \<open>dim_row (R n) = 2 ^ 1\<close>)
      qed
    qed
